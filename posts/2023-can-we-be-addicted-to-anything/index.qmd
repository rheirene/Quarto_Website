---
title: "PSYC601-608: Are we over-using the term 'addiction'"
description: "Seminar"
author:
  - name: Rob Heirene
...
total_year_gganimate <- data_total_per_year_pubs %>%
   ggplot(aes(Year, n)) + 
   geom_point() + 
   geom_line(size = 1.5, color = "#289998") + 
  # Add year counting label
   geom_text(aes(x = 1984, y = 370, label = as.factor(Year)), hjust = 0, vjust = 0, col = "#289998", size = 15, family = "Reem Kufi") +
...
bar_chart_race_pubs_year <- 
 ...
 theme(
     ...
     plot.caption = element_text(color = "#289998",  hjust = 1.06,
                                    vjust = 0.2)
