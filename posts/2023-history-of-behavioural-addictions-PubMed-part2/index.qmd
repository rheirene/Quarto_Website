...
# Make the city name column consistent with paper dataset and remove any duplicates to avoid crazy errors when joining (I checked this doesn't result in mismatches):
...
data_locations_with_full_geo_location<- left_join(data_locations_with_city_country,
           world_cities_filtered,
           join_by(cities_of_authors)
           ) 


# PRIORITSE matched country over linked one as it can't really make a mistake with the matched one as it's simple text extraction, whereas the link from city to country can be fallible as there are multiple cities with the same name.
