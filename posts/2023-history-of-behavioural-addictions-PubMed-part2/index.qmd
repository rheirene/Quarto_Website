...
world_cities_filtered <-  world.cities %>%
  ...
  filter(!name %in% c("China", # There is a city in Mexico called China, and including this in the dataset needed to pick up any papers published in China and link them to this city!
                      ...
                      "Columbia", #  This leads to confusion between British Columbia and Colombia in the US. Easier just to remove rather than be inaccurate. Will also lead to the actual city, Vancouver typically, being picked up
                     ...
                     "Jordan", #  country name acts incorrectly linked to the Philippines
                     
                     )) %>% 
  # .A combination of city trailers is to keep the city where it seems like it can be saved:
  filter(!city_country %in% c("Sussex, Canada",# Only the UK one appears and this gets confused
                              ...
                              "Bedford, UK", #  location in Australia gets thanks to the UK incorrectly
...
# There is one important caveat to the above, and that is Georgia USA. This gets extracted as the country, but it's obviously the state in the US! Also, the city in Georgia, Athens gets links to Greece incorrectly.  As such, we directly recode these below.
 
# There are also a few key cities that are duplicated that I didn't select in the filtering above that will need directly linking via code to the right city and country, including Cambridge, New York, London, Oxford, Bristol, and Newport
...
# mutate(cities_of_authors  = casewhen(institution = contains("Georgia State") ~ "Atlanta",
#                                      institution = contains("University of Georgia") ~ "Athens"),
#        institution = contains("Georgia College") ~ "Milledgeville"))  %>% 
#     mutate(countries_of_authors   = casewhen(institution = 
#                                                    contains("Georgia State University") ~ "USA",
#                                              institution = 
#                                                                                  contains("University of Georgia") ~ "USA"
#                                              institution = contains("Georgia College") ~ "USA"))
