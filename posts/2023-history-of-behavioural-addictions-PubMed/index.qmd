...
# Find the first and last study for each "addiction":
...
dumbbell_plot_data$Label <- factor(dumbbell_plot_data$Label, levels = rev(label_order))
  ...
  # Plot data: 
dumbbell_plot_first_last <- dumbbell_plot_data %>% 
ggplot(aes(x= Year, y= Label)) +
  geom_line(aes(group = Label), 
            size = 1.5, 
            colour="#666666") +
    geom_point(aes(color=Range), size=4) +
...
labs(
     title = sprintf("<span style = 'color: #70A377;'>First</span> and <span style = 'color: #50B5C8;'>last</span> publications for each \"addiction\""),
     caption = "Rob Heirene (@rheirene)"
   ) 
  #  labs(
  #    title = "First and last publications for each \"addiction\"",  # Add header
  #    # subtitle = "",
  #    caption = "Rob Heirene (@rheirene)"
  #  ) +
  # transition_states(Range) 
dumbbell_plot_first_last

 ggplotly(dumbbell_plot_first_last)
```
