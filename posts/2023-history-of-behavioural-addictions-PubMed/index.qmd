...
data_total_per_year_pubs <- raw_data %>%
   ...
   count() %>%
   ungroup() %>%
  mutate(first_label = case_when(Year >= 1980 & Year < 2000 ~ sprintf("1980  \nPathological Gambling first \nadded to DSM (version III)"),
                               TRUE ~ ""))                      
...
total_year_gganimate <- data_total_per_year_pubs %>%
  ...
  geom_text(aes(x = 1970, y =100, label = first_label), hjust = 0, vjust = 0, alpha = 0.9, col = "#F5F7F0", size = 4, family = "Reem Kufi",) +
